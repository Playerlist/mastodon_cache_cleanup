...
/usr/local/bin/docker-compose run --rm web bundle exec bin/tootctl emoji purge --remote-only >> cleanup.txt
/usr/local/bin/docker-compose run --rm web bundle exec bin/tootctl media remove-orphans >> cleanup.txt
